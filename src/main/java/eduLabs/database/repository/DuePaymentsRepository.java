...
public interface DuePaymentsRepository extends JpaRepository<DuePaymentsTableModel, StudentClass> {

    @Query(value = "SELECT * FROM student_payment WHERE studentid = :studentId", nativeQuery = true)
    List<DuePaymentsTableModel> getDuePaymentsQuery(@Param("studentId") String studentId);
